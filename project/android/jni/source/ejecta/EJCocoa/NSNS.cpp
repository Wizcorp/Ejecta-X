...
static bool splitWithForm(const char* pStr, strArray& strs)
{
    ...
    do 
    {
        ...
        NS_BREAK_IF(content.length() == 0);

        int nPosLeft  = content.find('{');
        int nPosRight = content.find('}');

        // don't have '{' and '}'
        NS_BREAK_IF(nPosLeft == (int)std::string::npos || nPosRight == (int)std::string::npos);
        // '}' is before '{'
        NS_BREAK_IF(nPosLeft > nPosRight);

        std::string pointStr = content.substr(nPosLeft + 1, nPosRight - nPosLeft - 1);
        // nothing between '{' and '}'
        NS_BREAK_IF(pointStr.length() == 0);

        int nPos1 = pointStr.find('{');
        int nPos2 = pointStr.find('}');
        // contain '{' or '}' 
        NS_BREAK_IF(nPos1 != (int)std::string::npos || nPos2 != (int)std::string::npos);
...
NSRect NSRectFromString(const char* pszContent)
{
    NSRect result = NSRectZero;

    do 
    {
        NS_BREAK_IF(!pszContent);
        ...
        NS_BREAK_IF(nPointEnd == (int)std::string::npos);
        ...
        strArray pointInfo;
        NS_BREAK_IF(!splitWithForm(pointStr.c_str(), pointInfo));
        strArray sizeInfo;
        NS_BREAK_IF(!splitWithForm(sizeStr.c_str(), sizeInfo));
        ...
        result = NSRectMake(x, y, width, height);
    } while (0);

    return result;
}

NSPoint NSPointFromString(const char* pszContent)
{
    NSPoint ret = NSPointZero;

    do 
    {
        strArray strs;
        NS_BREAK_IF(!splitWithForm(pszContent, strs));
...
NSSize NSSizeFromString(const char* pszContent)
{
    NSSize ret = NSSizeZero;
